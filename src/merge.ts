        ...
        packages: [
          {
            package: flatten(
              inputs.map((input) => {
                const originalBaseDir = input.data.coverage.sources?.[0].source[0].$t ?? '';

                return flatten(
                  input.data.coverage.packages.map((packages) => {
...
function sumCoverageProperty(
  ...
  return inputs.reduce((count: number, input: InputData) => count + parseInt(input.data.coverage[property], 10), 0);
}
